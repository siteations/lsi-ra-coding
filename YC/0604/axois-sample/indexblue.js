...
var query = axios.get(subj)
  .then(function (response) {
    ...
    var ofArr = response.data.Result.map(item => {
      return item.SubjectText;
    })

    console.log('choosen subject: ', ofArr);
    ...
    var arrSubjectCall = ofArr.map(item =>{
       var paraObj =  {
        params: {
        op: 'GetSubjectTitles',
        subject: item,
    ...
    console.log('promise in action ', arrSubjectCall);
    ...
    return Promise.all(arrSubjectCall)
      .then(function(results){
        ...
        var allResults = results.map(res =>{
          ...
          return res.length > 0; //filter out the empty results

        });

        var allResFlat = [];
        allResults.forEach(arr=>{
          allResFlat.push(...arr); //add just the content of the values
        })
